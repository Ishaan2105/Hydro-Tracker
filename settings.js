...
/* ── 🧬 PERSONALIZED GOAL CALCULATOR LOGIC ── */
var currentRecommendedGoalLiters = 2.5;
...
function calculatePersonalizedGoal() {
    ...
    // 2. Convert Height to CM based on Conversion Unit Type
    let heightCm = 175;
    if (heightUnit === 'cm') {
    ...
    // 3. Hydration Recommendation Formula
    let baseMl = weightKg * 35; // 35ml per kg baseline

    // Gender adjustment
    if (gender === 'male') baseMl += 250;
    else if (gender === 'other') baseMl += 100;

    // Height adjustment (+15ml per 5cm above 165cm baseline)
    if (heightCm > 165) {
        baseMl += Math.max(0, Math.round((heightCm - 165) / 5) * 15);
    }

    // Clamp between 1.2L (1200ml) and 6L (6000ml)
    baseMl = Math.max(1200, Math.min(6000, Math.round(baseMl / 50) * 50));
    currentRecommendedGoalLiters = parseFloat((baseMl / 1000).toFixed(2));

    // Update UI Result
    const resultDisplay = document.getElementById('calc-recommendation-display');
    const formulaNote = document.getElementById('calc-formula-note');

    if (resultDisplay) {
        resultDisplay.innerHTML = `${currentRecommendedGoalLiters} L <small style="font-size:1rem; opacity:0.7;">(${baseMl} ml)</small>`;
    }
    ...
    if (formulaNote) {
        ...
        formulaNote.innerText = `Based on ${Math.round(weightKg)}kg weight, ${heightDisplay} height & ${gender} baseline.`;
    }

    // Preserve metrics object inside data
    if (data) {
        data.profileMetrics = {
            weight: weightVal,
            weightUnit,
            heightUnit,
            heightSingle: document.getElementById('calc-height-single')?.value,
            heightFt: document.getElementById('calc-height-ft')?.value,
            heightIn: document.getElementById('calc-height-in')?.value,
            gender
        };
    }
}

async function applyPersonalizedGoal() {
    const goalInput = document.getElementById('goal-val');
    if (goalInput) {
        goalInput.value = currentRecommendedGoalLiters;
    }
    await saveGlobalGoal();
}
